...
def add_ai_suggestion_event(request):
    try:
        ...
        req_start = data.get('start_date') or data.get('start_datetime')
        req_end = data.get('end_date') or data.get('end_datetime')
        ...
        start_dt = None
        if req_start:
            parsed_dt = parse_datetime(req_start)
            if parsed_dt:
                start_dt = parsed_dt
            else:
                parsed_d = parse_date(req_start)
                if parsed_d:
                    start_dt = datetime.datetime.combine(parsed_d, datetime.time(9, 0))

        if not start_dt:
            if hasattr(trip, 'start_date') and trip.start_date:
                start_dt = datetime.datetime.combine(trip.start_date, datetime.time(9, 0))
            else:
                start_dt = timezone.now()

        if timezone.is_aware(timezone.now()) and timezone.is_naive(start_dt):
            start_dt = timezone.make_aware(start_dt)

        end_dt = None
        if req_end:
            parsed_end_dt = parse_datetime(req_end)
            if parsed_end_dt:
                end_dt = parsed_end_dt
            else:
                parsed_end_d = parse_date(req_end)
                if parsed_end_d:
                    end_dt = datetime.datetime.combine(parsed_end_d, datetime.time(11, 0))

        if not end_dt:
            end_dt = start_dt + timezone.timedelta(hours=2)

        if timezone.is_aware(timezone.now()) and timezone.is_naive(end_dt):
